  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="visible" style={{ display: 'flex', gap: '24px', height: '100%' }}>
      ...
      <motion.div variants={slideInLeft} style={{ width: '380px', backgroundColor: 'var(--bg-card)', borderRadius: '12px', padding: '32px', display: 'flex', flexDirection: 'column' }}>
        ...
        <motion.div variants={staggerContainer} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '32px' }}>
          {pieData.map(d => (
            <motion.div key={d.name} variants={staggerItem} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      {/* Right: Stats */}
      <motion.div variants={staggerContainer} style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
